...
function DatePickerCell({ label, value, onChange, lang = 'tr', minISO, disabled = false, disabledText = '—', dark = false, popoverAlign, compact = false }) {
  const [open, setOpen] = React.useState(false);
    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <CalendarPopover
          ...
          lang={lang}
          align={popoverAlign || (ref.current?.getBoundingClientRect()?.left > (typeof window !== 'undefined' ? window.innerWidth * 0.5 : 9999) ? 'right' : 'left')}
          compact={compact}
...
function CalendarPopover({ value, minISO, lang, onPick, align = 'left', compact = false }) {
  ...
  // Position from the configured side. `compact` (mobile) forces a smaller
  // popover and align-based positioning even when the viewport is wide
  // (the popover lives inside a phone frame whose own overflow would clip it).
  const isMobile = compact || (typeof window !== 'undefined' && window.innerWidth < 520);
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: isMobile ? 2 : 4, marginBottom: 6 }}>
        {dowH.map(d => <div key={d} style={{ fontSize: isMobile ? 8.5 : 9.5, color: '#94A3B8', fontWeight: 800, letterSpacing: 1, textAlign: 'center', textTransform: 'uppercase' }}>{d}</div>)}
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            <button key={i} type="button"
              ...
              onClick={() => onPick(iso)}
              style={{
                padding: isMobile ? '6px 0' : '8px 0', borderRadius: 6, border: 'none',
                fontFamily: 'var(--font-mono)', fontSize: isMobile ? 11 : 12, fontWeight: 700,
